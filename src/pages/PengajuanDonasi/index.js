...
const PengajuanDonasi = (props) => {
    ...
    const getDataUser = async () => {
        setMail(
            await AsyncStorage.getItem('loginKey').then(
                res => axios.get(`http://ikhlasbantu.herokuapp.com/users/mail/${res}`).then(
    ...
    const uploadFoto = () => {
        ...
        setInfo(
            fetch(`http://ikhlasbantu.herokuapp.com/upload/donasi`, {
                method: "POST",
                body: formData
            }).then(
                res => res.json().then(
    ...
    const onBtnSave = async () => {
        if (!judul) {
            ...
            try {
                await axios.post(`http://ikhlasbantu.herokuapp.com/donasis`, data).then(
                    res => {
